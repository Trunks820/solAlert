...
### ~~1. 线程池+asyncio混合调度性能瓶颈~~ ✅ 已完成（队列架构替代）

**原问题**：
- 📍 `src/solalert/monitor/bsc_websocket_monitor.py`
- ❌ 每条消息都执行 `asyncio.run()` 创建/销毁event loop
- ❌ CPU抖动、GIL竞争、消息串行处理（线程池限制）

**✅ 已实施方案（2025-11-04）**：

**方案：队列缓冲 + 消费者并发架构**
```
WebSocket (同步) → ThreadPoolExecutor (2工作线程)
                         ↓
                  asyncio.PriorityQueue (1000容量)
                         ↓
                  3个异步Consumer并发处理
                         ↓
                  handle_proxy_event / handle_swap_event
```

**关键改进**：
- ✅ 减少线程池使用（5个 → 2个，仅用于WebSocket桥接）
- ✅ 事件缓冲（1000容量队列，削峰填谷）
- ✅ 优先级调度（内盘优先级0，外盘优先级1）
- ✅ 并发控制（3个Consumer限制并发，避免过载）
- ✅ 序列号机制（避免PriorityQueue的dict比较错误）

**性能提升**：
- ⚡ 高峰期缓冲能力：0 → 1000消息
- ⚡ 并发处理能力：有限制但可控（3并发 + 队列缓冲）
- ⚡ 内盘交易延迟降低（优先处理）
- ⚡ 避免消息丢失（队列满前告警）
...
**备注**：完全asyncio化（第2步）移至P1优先级，当前架构已满足需求
...
**1. 数据库异步化**
- 安装 `aiomysql==0.2.0`
- 创建 `AsyncDatabaseManager`
- 使用异步连接池（`aiomysql.create_pool`）
- 所有查询改为 `async def` + `await`

**2. Redis异步化**
- 使用 `redis.asyncio`（redis-py 5.0+已支持）
- 创建 `AsyncRedisClient`
- 所有操作改为 `await`

**3. Monitor完全异步**
- WebSocket主循环：`async for message in ws`
- 消息处理：`asyncio.create_task(handle_message())`
- 并发控制：`asyncio.Semaphore(100)`
- 移除线程池（`ThreadPoolExecutor`）

**4. 通知发送异步化**
- Telegram API已是异步（`python-telegram-bot`）
- 确保所有 `send_message` 都是 `await`
...
| 优化项 | 实际工作量 | 收益 | 状态 |
|--------|----------|------|------|
| ~~1. 队列架构替代线程池~~ | 1天 | 高峰缓冲+优先级调度+并发控制 | ✅ 已部署 |
